...
def read_setting():
	with winreg.OpenKey(winreg.HKEY_CURRENT_USER,"Software\Microsoft\Windows\CurrentVersion\Internet Settings") as handle:
...
def set_proxy(addr,port,bit):
	with winreg.OpenKeyEx(winreg.HKEY_CURRENT_USER,"Software\Microsoft\Windows\CurrentVersion\Internet Settings", reserved=0, access=winreg.KEY_SET_VALUE) as handle:
		if len(addr):
...
def show_menu():
	...
	for i in menu_entry:
		ttk.Button(lframe2, text=config_str[i][0], command = lambda i1=i: (set_proxy(config_str[i1][1], config_str[i1][2], config_str[i1][3])), style="TButton").grid(row=int(i))
	return
